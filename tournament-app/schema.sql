-- Slaughter Games MTG Draft Tournament App Schema (Supabase/PostgreSQL)
-- Compatible with Netlify + Supabase JS client
...
-- Snapshot standings per round (auditability + historical ladder)
...
-- Track opponent graph (for rematch checks + tie-breakers)
...
create index if not exists idx_matches_tournament_round on public.matches(tournament_id, round_no);
create index if not exists idx_opponents_tournament_player on public.player_opponents(tournament_id, player_id);
create index if not exists idx_stats_tournament_round on public.player_round_stats(tournament_id, round_no);
...
using (
  exists (
    select 1 from public.tournaments t
    where t.id = player_round_stats.tournament_id
      and t.owner_id = auth.uid()
  )
)
with check (
...
using (
  exists (
    select 1 from public.tournaments t
    where t.id = player_opponents.tournament_id
      and t.owner_id = auth.uid()
  )
)
with check (
